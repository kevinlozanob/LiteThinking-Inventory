...
def generar_pdf_inventario(productos):
    ...
    for prod in productos:
        print(f"DEBUG: Producto {prod.codigo}, Precios: {prod.precios}, Tipo: {type(prod.precios)}") # Debug
        
        precio_str = "N/A"
        
        # Validamos que 'precios' sea un diccionario y no esté vacío
        if isinstance(prod.precios, dict) and prod.precios:
            # Busca USD, si no, toma la primera moneda disponible
